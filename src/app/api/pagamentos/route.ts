...
export async function POST(request: NextRequest) {
  if (!mercadoPagoConfigurado()) {
    return Response.json(
      { ok: false, erro: "Pagamento online não configurado." },
      { status: 503 },
    );
  }
  ...
  if (!analise.success) {
    return Response.json(
      { ok: false, erro: "Dados de pagamento inválidos." },
      { status: 400 },
    );
  }
  ...
  if (!data) {
    return Response.json(
      { ok: false, erro: "Pedido não encontrado." },
      { status: 404 },
    );
  }
  ...
  if (pedido.status === "cancelado") {
    return Response.json(
      { ok: false, erro: "Este pedido foi cancelado." },
      { status: 409 },
    );
  }
  ...
  try {
    resultado = await criarPagamento(
      ...
      {
        ...dados,
        issuer_id: dados.issuer_id ? String(dados.issuer_id) : undefined,
      },
    ...
    return Response.json(
      {
        ok: false,
        erro: "Não consegui falar com o Mercado Pago. Tente de novo.",
      },
      { status: 502 },
    );
  }
  ...
  await supabase
    ...
    .update({
      ...
      status_pagamento: resultado.aprovado
        ? "pago"
        : resultado.pendente
          ? "pendente"
          : "falhou",
      status:
        resultado.aprovado && pedido.status === "aguardando_pagamento"
          ? "recebido"
          : pedido.status,
    })
    .eq("id", pedido.id);
  ...
  return Response.json({
    ...
    erro:
      resultado.aprovado || resultado.pendente
        ? undefined
        : explicarRecusa(resultado.detalhe),
  });
}
