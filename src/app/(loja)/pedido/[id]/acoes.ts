...
import { consultarPagamento, mercadoPagoConfigurado, urlBase } from '@/lib/mercadopago'
...
export async function verificarPagamentoAction(
  ...
  if (!pedido.mp_payment_id) {
    return { pago: false, mensagem: 'Ainda não há pagamento iniciado para este pedido.' }
  }
  ...
  try {
    ...
    return { pago: false, mensagem: 'Não consegui consultar agora. Tente em instantes.' }
  }

  if (!pagamento) {
    return { pago: false, mensagem: 'Pagamento não encontrado no Mercado Pago.' }
  }
  ...
  if (pagamento.valorCentavos < pedido.total_centavos) {
    ...
    return { pago: false, mensagem: 'O valor pago não confere. Fale com o restaurante.' }
  }
  ...
  return { pago: true, mensagem: 'Pagamento confirmado! Seu pedido entrou na fila.' }
}
